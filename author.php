                ...
                <div class="col-md-8">
                    ...
                    <div class="post-container">
                        <?php 
                            ...
                            $query2   = "SELECT * FROM users WHERE user_id = '{$auth_id}'";
                            $result2  = mysqli_query($connection, $query2) or die("Query Failed");

                            $row2 = mysqli_fetch_assoc($result2);
                        ?>
                            <h2 class="page-heading"><?php echo strtoupper($row2['username']); ?></h2>
                        <?php
                            ...
                            $query = "SELECT posts.post_id, posts.title, posts.post_date, posts.category, posts.description, posts.post_img, posts.author, categories.category_name, categories.category_id, users.username FROM posts 
                        ...
                        <?php 
                            ...
                            $query1 = "SELECT * FROM posts WHERE posts.author = '{$auth_id}'";
                            ...
                            if(mysqli_num_rows($result1)){
                                ...
                                echo "<ul class='pagination admin-pagination'>";
                                    if($page_number > 1){
                                        echo '<li><a href="author.php?auth_id='.$auth_id.'&page='.($page_number - 1).'">prev</a></li>';
                                    }
                                    for($i=1; $i<=$total_page; $i++){
                                        ...
                                        echo '<li class='.$active.'><a href="author.php?auth_id='.$auth_id.'&page='.$i.'">'.$i.'</a></li>';
                                    }
                                    if($total_page > $page_number){
                                        echo '<li><a href="author.php?auth_id='.$auth_id.'&page='.($page_number + 1).'">next</a></li>';
                                    }
                                echo "</ul>";
                            }
                        }
